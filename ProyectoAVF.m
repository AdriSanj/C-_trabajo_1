...
N=3000;                  % Numero de puntos del mallado
x=linspace(-5,5,N);     % Vector de mallado
t=0;                    % Instante de tiempo inicial
mu=0.9;          
Dx=(x(N)-x(1))/N;       % Espacio entre puntos del mallado
Dt=Dx*mu/c0;  % Espacio entre tiempos. Esta no tocarla, depende de mu y Dx
it_temp=40;     % Numero de nodos espaciales que queremos.  
tfin=Dt*it_temp;        % Tiempo final de ejecucion 
...
% elegir "no". Cualquier otra opcion no imprime las graficas.
comparativa="si";
...
elseif apartado=="er" | apartado=="ec"      % Ejecuta esta parte de codigo 
    ...
    u_i=u_i_1;
    p_i=p_i_1;
    while t<tfin           %El tiempo maximo elegido es suficiente para ver el comportamiento de las funciones
        for i=2:N-1   
    ...
    if comparativa=="si"
        fprintf("Error de aproximacion de la velocidad u")
        err_u_1=error_un(u_i_1,U,Dx)        % error en norma 1
        err_u_2=error_inf(u_i_1,U)          % error en norma infinito
        fprintf("Error relativo de aproximacion de la presion p")  % hacemos el error relativo para apreciar el error respecto al valor inicial pl, que es del orden de 1e6
        err_p_1=error_un(p_i_1,P,Dx)/pl     % error "relativo" en norma 1
        err_p_2=error_inf(p_i_1,P)/pl       % error "relativo" en norma infinito   
    end
else 
    fprintf("No has seleccionado una opcion correcta")
end
...
function err= error_un(X,Y,d)
    err=0;          % d es el interespaciado
    for i=1:size(X,2)
        err=err+abs(X(i)-Y(i));
    end
    err=err*d;
end
function err=error_inf(X,Y)
    err=max(abs(X-Y));
end
